...
namespace ET.Core.Common
{
	...
	public class NullableConverter : TypeConverterBase
	{
			...
			: base(type)
		{
			...
			{
				throw ErrorHelper.Argument("type", "Type is not a nullable type.");
			}

			_underlyingTypeIsConvertible = typeof(IConvertible).IsAssignableFrom(UnderlyingType) && !UnderlyingType.GetTypeInfo().IsEnum;
